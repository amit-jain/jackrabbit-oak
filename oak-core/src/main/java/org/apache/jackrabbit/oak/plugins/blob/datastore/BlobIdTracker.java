...
public class BlobIdTracker implements Closeable, BlobTracker {
    ...
    static class Store implements Closeable {
        ...
        Store(File rootDir, String prefix) throws IOException {
            ...
            processFile =
                fileTreeTraverser().breadthFirstTraversal(rootDir).firstMatch(IN_PROCESS.filter()).orNull();
        ...
        protected void removeRecords(File recs) throws IOException {
            ...
            try {
                ...
                try {
                    iterator = new FileLineDifferenceIterator(recs, getBlobRecordsFile());
                    writeStrings(iterator, temp);
                } finally {
                    closeQuietly(writer);
                    if (iterator != null) {
                        iterator.close();
